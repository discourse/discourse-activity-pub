...
RSpec.describe DiscourseActivityPub::Auth::Gotosocial do
  let!(:domain1) { "gotosocial.example.com" }
  let!(:redirect_uri) do
  ...
  describe "create_client" do
    it "sends the right request to register a client" do
      expect_request(
        domain: domain1,
        path: DiscourseActivityPub::Auth::Gotosocial::APP_PATH,
        body: app_request_body,
      )
      DiscourseActivityPub::Auth::Gotosocial.create_client(domain1)
    end
    ...
    context "with a successful client response" do
      ...
      context "with a successful client token response" do
        before do
          expect_request(
            domain: domain1,
            path: DiscourseActivityPub::Auth::Gotosocial::TOKEN_PATH,
      ...
      context "with an unsuccessful client token response" do
        ...
        it "does not save a client" do
          expect { DiscourseActivityPub::Auth::Gotosocial.create_client(domain1) }.not_to change {
            DiscourseActivityPubClient.count
          }
        end

        it "returns nil" do
          expect(DiscourseActivityPub::Auth::Gotosocial.create_client(domain1)).to eq(nil)
        end

        it "adds returned errors to the instance" do
          auth = DiscourseActivityPub::Auth::Gotosocial.new(domain: domain1)
          auth.create_client
  ...
  describe "#get_authorize_url" do
    it "returns nil without a client for the domain" do
      expect(DiscourseActivityPub::Auth::Gotosocial.get_authorize_url(domain1)).to eq(nil)
    end

    context "with a client for the domain" do
      let!(:client) do
        Fabricate(
          :discourse_activity_pub_client_gotosocial,
          domain: domain1,
          credentials: {
            client_id: client_id,
            client_secret: client_secret,
      ...
      it "returns an authorize url" do
        expect(DiscourseActivityPub::Auth::Gotosocial.get_authorize_url(domain1)).to eq(
          # https://docs.joinmastodon.org/methods/oauth/#query-parameters
          "https://gotosocial.example.com/oauth/authorize?client_id=#{client_id}&response_type=code&redirect_uri=#{CGI.escape(redirect_uri)}&scopes=#{CGI.escape(DiscourseActivityPub::Auth::Gotosocial::SCOPES)}&force_login=true",
        )
      end
    end
  end
  ...
  describe "#get_token" do
    context "without a client for the domain" do
      it "returns nil" do
        expect(DiscourseActivityPub::Auth::Gotosocial.get_token(domain1, { code: code })).to eq(nil)
      end
    end
    ...
    context "with a client for the domain" do
      ...
      context "with a successful response" do
        ...
        it "returns an access token" do
          expect(DiscourseActivityPub::Auth::Gotosocial.get_token(domain1, { code: code })).to eq(
            access_token,
          )
        end
      end
      ...
      context "with an error response" do
        ...
        it "adds errors to the instance" do
          oauth = DiscourseActivityPub::Auth::Gotosocial.new(domain: domain1)
    ...
    context "with a successful account response" do
      before do
        expect_request(
          domain: domain1,
          verb: :get,
          path: DiscourseActivityPub::Auth::Gotosocial::ACCOUNT_PATH,
          headers: {
      ...
      it "returns an actor id" do
        expect(DiscourseActivityPub::Auth::Gotosocial.get_actor_ap_id(domain1, access_token)).to eq(
