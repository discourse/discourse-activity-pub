...
RSpec.describe DiscourseActivityPub::Auth::OAuth do
  ...
  describe "create_app" do
    ...
    context "with an error response" do
      ...
      it "returns nil" do
        expect(DiscourseActivityPub::Auth::OAuth.create_app(domain1)).to eq(nil)
      end
  ...
  describe "#get_authorize_url" do
    it "returns nil without an app for the domain" do
      expect(DiscourseActivityPub::Auth::OAuth.get_authorize_url(domain1)).to eq(nil)
    end

    context "with an app for the domain" do
      before do
        PluginStore.set(
          DiscourseActivityPub::Auth::OAuth::PLUGIN_STORE_KEY,
          domain1,
          app_response_body,
        )
      end
      ...
      it "returns an authorize url" do
        expect(DiscourseActivityPub::Auth::OAuth.get_authorize_url(domain1)).to eq(
  ...
  describe "#get_token" do
    context "without an app for the domain" do
      it "returns nil" do
        expect(DiscourseActivityPub::Auth::OAuth.get_token(domain1, code)).to eq(nil)
      end
    end
    ...
    context "with an app for the domain" do
      ...
      context "with a successful response" do
        ...
        it "returns an access token" do
          expect(DiscourseActivityPub::Auth::OAuth.get_token(domain1, code)).to eq(access_token)
        end
      end
  ...
  describe "#get_actor_id" do
    context "with a successful account response" do
      ...
      it "returns an actor id" do
        expect(DiscourseActivityPub::Auth::OAuth.get_actor_id(domain1, access_token)).to eq(
    ...
    context "with an error response" do
      ...
      it "returns nil" do
        expect(DiscourseActivityPub::Auth::OAuth.get_actor_id(domain1, access_token)).to eq(nil)
      end
