...
RSpec.describe Category do
  ...
  describe "#save_custom_fields" do
    ...
    it "raises if username is invalid" do
      expect do toggle_activity_pub(category, username: "abc象") end.to raise_error(
    ...
    it "raises if username is not unique for local actors" do
      ...
      expect do toggle_activity_pub(category, username: "general") end.to raise_error(
        ActiveRecord::Rollback,
      )
    end

    it "publishes activity pub state if activity_pub_enabled is changed" do
      message = MessageBus.track_publish("/activity-pub") { toggle_activity_pub(category) }.first
